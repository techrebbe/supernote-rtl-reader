...
The read-only marker sets `editable=false`. In that mode v0.0.67
...
read-only. v0.0.67 performs the full PDF and snapshot hashing on a background
...
v0.0.67 also clears the destroyed activity reference and recycles all
per-activity full-resolution page, ink, and digest bitmaps when the native
reader closes.

For canonical landscape lasso moves, v0.0.67 converts the translated origin
...
v0.0.67 compiles and passes automated handshake, backup-attestation,
