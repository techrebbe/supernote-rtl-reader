...
static inline int loader_positive_fixture_case(int which) {
    return which == 0 || which == 1 || which == 12 || which == 17 || which == 18;
}
static inline size_t loader_expected_report_bytes(int which, int crash_after) {
    if (crash_after >= 0) return 0;
    if (which == 11) return 2u * sizeof(struct loader_result);
    return (loader_positive_fixture_case(which) ? 4u : 3u) * sizeof(struct loader_result);
}
static inline int loader_sequence(const struct loader_result *r,size_t bytes,
    ...
    if(!r || which<0 || which>18 || bytes%sizeof(*r) || bytes<2*sizeof(*r) ||
    ...
    if(loader_positive_fixture_case(which))
