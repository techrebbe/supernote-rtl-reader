...
geometry. Native module v0.0.20 authenticates each internal link's target-view
policy and the exact source snapshot. It rejects non-integral manifest page
indices or output sizes, numeric strings/non-finite values in spread or link
geometry, transformed URI `/IsMap true` actions, and link geometry outside the
...
together before opening them with v0.0.20. Native reader callbacks perform only
strong PDF and sidecar
