...
`page-143-native-viewport-v1.json` is the exact seven-field native viewport descriptor expected for zero-based Virtual Spread page 1 on the Nomad's 1872-by-1404 persistent page canvas. It is a cross-project golden vector, not runtime authority: production InkBridge must still obtain a fresh matching descriptor from the v0.0.26 companion provider and validate it against `PluginFileAPI.getPageSize` and the active schema-v3 evidence.
